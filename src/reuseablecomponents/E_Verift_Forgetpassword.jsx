...
const E_Verify_ForgetPassword = () => {
  ...
  const [sendCode, { isLoading, isSuccess, isError, error }] = useSendCodeMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormErrors({
        ...formErrors,
        general: err.data?.message || 'Failed to send verification code. Please try again.'
      });
    }
  };
      ...
      <div className="w-full lg:w-max-w-7xl flex items-center justify-center relative p-4 md:p-0">
        <div className="border-2 border-black p-3 w-full max-w-md px-4 md:px-8">
          <div className="mb-8 text-center md:text-left">
            ...
            <h2 className="text-xl md:text-2xl font-semibold">Email Verification</h2>
          </div>

          {isError && (
            <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
              <p>{error?.data?.message || 'Failed to send verification code. Please try again.'}</p>
            </div>
          )}
